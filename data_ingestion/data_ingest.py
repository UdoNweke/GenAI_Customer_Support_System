...
class ingest_data:
    ...
    def __init__(self):
        #print("data ingestion class has init method--")
    ...
    def data_ingestion(self, status):
        ...
        if storage == None:
            doc = self.data_converter.data_transformation()
            inserted_ids = vstore.add_documents(doc)
            print(inserted_ids)
        else:
            return vstore
...
if __name__ == '__main__':
    ...
    results = vstore.similarity_search("can you give the result of low budget headphones")
    for res in results:
